...
async function handleAttackButton(interaction, client) {
  ...
  // Old behavior: damage should stay around 1-5 regardless of attack level.
  const minDmg = 1;
  const maxDmg = 5;
  ...
  const monsterDb = require('./db').monsterDb;
  await run(monsterDb,
  ...
  const imageName = require('./monsterState').getMonsterImageName(monster.title);
  ...
  if (newHp <= 0) {
    ...
    const rewardDb = require('./db').monsterDb;
    ...
    const historyDb = require('./db').monsterDb;
    await run(
      historyDb,
    ...
    const configDb = require('./db').monsterDb;
    const channelId = await getConfig(configDb, keys.MONSTER_CHANNEL_ID, process.env.MONSTER_CHANNEL_ID);
    if (channelId) {
