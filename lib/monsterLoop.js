...
async function startMonsterLoop(client) {
  await init();
  await ensureDefaults({ monsterDb: require('./db-adapter').getMonsterDb() });
  ...
  const channelId = await getConfig(require('./db-adapter').getMonsterDb(), keys.MONSTER_CHANNEL_ID, null);
  if (channelId) {
