...
function clampMonsterImageTier(maxHp) {
  // tiers by HP as requested:
  // 1-19 => #1
  // 20-39 => #2
  // 40-69 => #3
  // 70-109 => #4
  // 110+ => #5
  if (maxHp <= 19) return 1;
  if (maxHp <= 39) return 2;
  if (maxHp <= 69) return 3;
  if (maxHp <= 109) return 4;
  return 5;
}

function imageUrlFromTier(tier) {
  // If you want real URLs, replace with CDN URLs.
  // For now, Discord usually needs an attachment or public URL.
  // This bot will attempt to reference images as public by using file:// is NOT allowed.
  // So we keep placeholders and you should replace with real URLs or upload to a CDN.
  // We'll default to a local relative path string.
  return `attachment://monster${tier}.png`;
}
...
async function getCurrentMonster() {
  ...
  const tier = clampMonsterImageTier(maxHp);
  return {
    monsterId,
    hp,
    maxHp,
    title: baseTitle,
    imageTier: tier,
    imageUrl: imageUrlFromTier(tier)
  };
}
