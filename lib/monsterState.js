...
async function getMonsterStateValue(key, defaultValue = null) {
  ...
  const row = await get(db, `SELECT value FROM monster_state WHERE key = ?`, [key]);
...
async function resetMonsterState() {
  const db = require('./db').monsterDb;
  await run(db, `DELETE FROM monster_state`);
...
async function spawnNewMonster({ hardReset = false } = {}) {
  ...
  const db = require('./db').monsterDb;
  await run(db, `DELETE FROM monster_attacks WHERE monsterId = ?`, [monsterId]).catch(() => {});
