...
const { getPlayer, ensurePlayer, spendEnergy, addXP, addGold, addGems, addPlayerResource, addKnowledgeBook, getPlayerWorkLevels } = require('./players');
...
function calculateResourceReward(regionId, weather, timeBonus, playerLevel, workLevels = {}) {
  const region = REGIONS.find(r => r.id === regionId);
  if (!region) {
  ...
  let resource = 'wood';
  let amount = 2 + Math.floor(playerLevel / 10);

  if (regionId === 'desert') {
    resource = 'stone';
    amount = 2 + Math.floor(playerLevel / 12);
  } else if (regionId === 'volcano') {
    resource = 'iron';
    amount = 2 + Math.floor(playerLevel / 14);
  } else if (regionId === 'ice') {
    resource = 'gems';
    amount = 1 + Math.floor(playerLevel / 20);
  }

  const workBonus = (workLevels.lumberjack || 0) + (workLevels.miner || 0) + (workLevels.jeweler || 0);
  amount += workBonus;

  if (resource === 'wood' && (workLevels.lumberjack || 0) > 0) {
    amount += workLevels.lumberjack;
  }
  if (resource === 'stone' && (workLevels.miner || 0) > 0) {
    amount += workLevels.miner;
  }
  if (resource === 'gems' && (workLevels.jeweler || 0) > 0) {
    amount += workLevels.jeweler;
  }

  amount = Math.max(1, Math.floor(amount * (weather?.battleBonus || 1) * (timeBonus?.bonus || 1)));
...
async function exploreRegion(userId, regionId) {
  ...
  const workLevels = await getPlayerWorkLevels(userId);
  const resourceReward = calculateResourceReward(regionId, weather, timeBonus, player.level, workLevels);
  await addPlayerResource(userId, resourceReward.resource, resourceReward.amount);
  
  // Small chance to find gems
  const gemChance = 0.1 + (weather.id === 'aurora' ? 0.2 : 0);
  let foundGem = Math.random() < gemChance;
  if (foundGem) {
    await addGems(userId, 1);
  }

  const knowledgeBookChance = 0.03 + (workLevels.engineer || 0) * 0.005;
  let foundKnowledgeBook = Math.random() < knowledgeBookChance;
  if (foundKnowledgeBook) {
    await addKnowledgeBook(userId, 1);
  }
...
async function getDailyDungeon() {
  ...
  const dungeon = await get(require('./db-adapter').getMonsterDb(),
    { date: today }
  );
    ...
    await run(require('./db-adapter').getMonsterDb(),
      {
        ...
        rewards: JSON.stringify(dungeonData.rewards)
      }
    );
