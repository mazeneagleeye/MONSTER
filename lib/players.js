...
async function addKnowledgeBook(userId, amount = 1) {
  await run(require('./db').playersDb,
    `UPDATE players SET knowledgeBooks = knowledgeBooks + ? WHERE userId = ?`,
    [amount, userId]
  );
}
...
async function getLeaderboard(limit = 10) {
  const rows = await all(require('./db').playersDb,
    `SELECT userId, CAST(totalDamage AS INTEGER) as totalDamage FROM players ORDER BY CAST(totalDamage AS INTEGER) DESC LIMIT ?`,
    [Number(limit)]
  );
  return rows;
}
