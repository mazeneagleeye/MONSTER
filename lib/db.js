...
function saveDatabases() {
  if (monsterDb) {
    ...
    fs.writeFileSync(path.join(__dirname, '..', 'monster.db'), buffer);
  }
  
  if (playersDb) {
    const data = playersDb.export();
    const buffer = Buffer.from(data);
    fs.writeFileSync(path.join(__dirname, '..', 'players.db'), buffer);
  }
}
...
function all(db, sql, params = []) {
  return new Promise((resolve, reject) => {
    try {
      const stmt = db.prepare(sql);
      // sql.js statement doesn't always support stmt.all(); use stmt.get repeatedly.
      const results = [];
      while (true) {
        const row = stmt.step() ? stmt.getAsObject(params) : stmt.getAsObject ? stmt.getAsObject(params) : null;
        if (!row) break;
        results.push(row);
      }
      stmt.free();
