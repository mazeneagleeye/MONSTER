...
async function run(collectionOrDb, sql, params = [], options = {}) {
  if (currentDb === 'mongodb') {
    ...
    // Use SQLite
    return await sqliteRun(resolveSqliteDb(collectionOrDb), sql, params);
  }
}
...
async function get(collectionOrDb, sql, params = []) {
  if (currentDb === 'mongodb') {
    ...
    if (typeof sql === 'object' && sql !== null) {
      ...
      return await sqliteGet(resolveSqliteDb(collectionOrDb), query.sql, query.params);
    }
    return await sqliteGet(resolveSqliteDb(collectionOrDb), sql, params);
  }
}
...
async function all(collectionOrDb, sql, params = []) {
  if (currentDb === 'mongodb') {
    ...
    if (typeof sql === 'object' && sql !== null) {
      const query = objectQuery(collectionOrDb, sql);
      return await sqliteAll(resolveSqliteDb(collectionOrDb), query.sql, query.params);
    }
    return await sqliteAll(resolveSqliteDb(collectionOrDb), sql, params);
  }
}

function resolveSqliteDb(collectionOrDb) {
  if (typeof collectionOrDb !== 'string') return collectionOrDb;
  if (collectionOrDb === 'players' || collectionOrDb === 'guilds' || collectionOrDb === 'player_monsters') {
...
async function sqliteObjectRun(db, document, options = {}) {
  ...
  const sqliteDb = resolveSqliteDb(db);
  if (document.$delete) {
    const filter = Object.fromEntries(Object.entries(document).filter(([key]) => key !== '$delete'));
    const query = objectWhere(filter);
    return await sqliteRun(sqliteDb, `DELETE FROM ${table} WHERE ${query.sql}`, query.params);
  }
  if (document.$set || document.$inc) {
    ...
    const query = objectWhere(filter);
    return await sqliteRun(sqliteDb, `UPDATE ${table} SET ${assignments.join(', ')} WHERE ${query.sql}`, [...params, ...query.params]);
  }
  const fields = Object.keys(document);
  const placeholders = fields.map(() => '?').join(', ');
  return await sqliteRun(sqliteDb, `INSERT OR IGNORE INTO ${table} (${fields.join(', ')}) VALUES (${placeholders})`, fields.map(field => document[field]));
}
